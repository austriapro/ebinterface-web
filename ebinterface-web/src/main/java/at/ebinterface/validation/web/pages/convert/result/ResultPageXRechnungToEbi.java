package at.ebinterface.validation.web.pages.convert.result;

import org.apache.wicket.markup.html.WebPage;

import at.ebinterface.validation.web.pages.BasePage;

/**
 * Used to show the results of the conversion from XRechnung to ebInterface
 *
 * @author Philip Helger
 */
public final class ResultPageXRechnungToEbi extends BasePage
{
  public ResultPageXRechnungToEbi (final byte [] pdf,
                                   final byte [] xml,
                                   final String log,
                                   final Class <? extends WebPage> returnPage)
  {
    final ResultPanelXRechnungToEbi resultPanel = new ResultPanelXRechnungToEbi ("xRechnungToEbiResultPanel",
                                                                                 pdf,
                                                                                 xml,
                                                                                 log,
                                                                                 returnPage);
    add (resultPanel);
  }
}
