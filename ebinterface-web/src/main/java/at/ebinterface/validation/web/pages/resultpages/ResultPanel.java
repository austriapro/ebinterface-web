...
public final class ResultPanel extends Panel
{
  public ResultPanel (final String id,
                      ...
                      @Nullable final byte [] pdf,
                      @Nullable final byte [] xml,
                      ...
                      @Nullable final Class <? extends WebPage> returnPage)
  {
    ...
    if (validationResult.getDeterminedEbInterfaceVersion () == null ||
        !validationResult.getDeterminedEbInterfaceVersion ().isSigned () ||
        !VerificationServiceInvoker.isActivated ())
    {
      signatureResultContainer.setVisibilityAllowed (false);
    }

    // Get the result details for the signature
    final SignatureValidationResult signatureValidationResult = new SignatureValidationResult (validationResult.getVerifyDocumentResponse ());

    // Signature
    signatureResultContainer.add (new SignatureDetailsPanel ("signatureDetails",
                                                             Model.of (Boolean.valueOf (signatureValidationResult.isSignatureValid ())),
                                                             Model.of (signatureValidationResult.getSignatureText ())));

    // Certificate
    signatureResultContainer.add (new SignatureDetailsPanel ("certificateDetails",
                                                             Model.of (Boolean.valueOf (signatureValidationResult.isCertificateValid ())),
                                                             Model.of (signatureValidationResult.getCertificateText ())));

    // Manifest
    signatureResultContainer.add (new SignatureDetailsPanel ("manifestDetails",
                                                             Model.of (Boolean.valueOf (signatureValidationResult.isManifestValid ())),
                                                             Model.of (signatureValidationResult.getManifestText ())));
